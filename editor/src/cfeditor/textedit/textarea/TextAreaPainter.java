 ...
 * @version $Id: TextAreaPainter.java,v 1.1.1.1 2003/10/16 00:23:06 michtoen Exp $
 */
public class TextAreaPainter extends JComponent implements TabExpander {
	...
	protected FontMetrics fm;
        protected boolean needMetricsUpdate; // need update for font metrics when true
        protected boolean needLineUpdate;    // need update for line height when true
        protected int defaultLineHeight;
        protected int defaultCharWidth;
	...
	public TextAreaPainter(JEditTextArea textArea, TextAreaDefaults defaults)
	{
		...
		eolMarkers = defaults.eolMarkers;
                needMetricsUpdate = true; // need to update font metrics before drawing
                needLineUpdate = true;
	}
        ...
        public int getDefaultLineHeight() {
            if (needLineUpdate) {
                /*
                if (!getFont().hasUniformLineMetrics())
                    System.out.println("Warning: Font has no uniform line metrics.");
                if (getFont().getStyle() != Font.PLAIN)
                    System.out.println("No plain font used");*/

                defaultCharWidth = (int)(Math.round((new TextLayout("WgGhdJj", getFont(),
                                         new FontRenderContext(new java.awt.geom.AffineTransform(), false, false))).getAdvance()/7.));
                defaultLineHeight = Math.round(getFont().getLineMetrics("WgGhdJj", new FontRenderContext(new java.awt.geom.AffineTransform(), false, false)).getHeight());
			...
			PlainDocument.tabSizeAttribute)).intValue();
                */
                if (needMetricsUpdate) {
                    fm = gfx.getFontMetrics(getFont());
                    textArea.recalculateVisibleLines();
                    needMetricsUpdate = false;
                }

                tabSize = fm.charWidth(' ') * 4; // I want 4-space wide tabs
                //TextLayout tl = new TextLayout("    ", getFont(), ((Graphics2D)gfx).getFontRenderContext());
                //tabSize = Math.round(tl.getAdvance());
	...
	public Dimension getMinimumSize()
	{
        ...
        /**
         * This is the first method which gets directly called from paint().
         * All lines which are sceduled for redraw get drawn with this method.
         */
		...
		Color defaultColor, int x, int y)
	{
        ...
        /**
         * When syntax highlighting is enabled, this method is called from
         * paintLine() to do the actual drawing of the line.
         */
		...
		int line, Font defaultFont, Color defaultColor, int x, int y)
	{
		...
		if(eolMarkers) {
                    // draw End-Of-Line markers, if enabled
                    gfx.setColor(eolMarkerColor);
                    gfx.drawString(".",x,y);
		}
	}
	...
	protected void paintHighlight(Graphics gfx, int line, int y)
	{
		...
		if(line == textArea.getCaretLine()) {
			paintCaret(gfx,line,y); // CARET gets drawn at this point (if currently visible)
                }
	}
	...
	protected void paintCaret(Graphics gfx, int line, int y) {
            if(textArea.isCaretVisible()) {
                ...
                if(textArea.isOverwriteEnabled()) {
                    ...
                    if (caretWidth <= 1) {
                        // This special case is very important: If 'caretWidth = 1' below rectangle would
                        // have zero width. With JDK 1.4.1, such a rect with zero width does not get drawn,
                        // which leads to incredible problems. (Damn this was hard to figure out!)
