 ...
 */
package cfeditor.textedit.textarea;
 ...
 *
 * @version $Id: JEditTextArea.java,v 1.1.1.1 2003/10/16 00:23:02 michtoen Exp $
 */
public class JEditTextArea extends JComponent
{
	...
	public JEditTextArea(TextAreaDefaults defaults)
	{
        ...
        /**
         * In JDKs above 1.4, the tab key is used for focus traversal. That means the tab
         * key normally does not "work" inside the text area. But that would be a pity,
         * because we need the tab key for indentation.
         *
         * So what this method does is setting the focus traversal to "tab + <control>",
         * in order to "free" the tab key from focus traversal events.
         * In JDKs above 1.4, this task can be accomplished simply by three lines of code:
         * <code>
         ...
         * The problem is that the class "KeyboardFocusManager" as well as the method
         * "setFocusTraversalKeys" are undefined in JDKs 1.3.* and below. Hence, if above
         * code was inserted as-is, this application would no longer compile with java 1.3.
         *
         * The solution to this problem is implemented here:
         * The critical classes and methods are accessed through the reflection interface,
         * which allows to execute them *if defined* but still compile *if undefined*.
         */
        private void freeTabKeyFromFocusTraversal() {
            try {
                ...
                java.lang.reflect.Field field = Class.forName("java.awt.KeyboardFocusManager").getField("FORWARD_TRAVERSAL_KEYS");
                Integer value = new Integer(field.getInt(field)); // store the value of this field
                ...
                if (methods != null) {
                    for (int i=0; i<methods.length; i++) {
                        ...
                        if (methods[i].getName().equalsIgnoreCase("setFocusTraversalKeys")) {
                            methods[i].invoke(this, new Object[] {value, forwardTraversalKeys});
                            // System.out.println("freeTabKeyFromFocusTraversal() succeeded!");
                        }
                    }
                }
            }
            catch (Exception e) {
                // If something is undefined which was tried to access above, an exception
                // is thrown and we end up here. But that's okay, it is intended to happen
                // for all JDKs 1.3.*. Those older JDKs don't need to set focus traversal anyways.

                // System.out.println("freeTabKeyFromFocusTraversal() failed: "+e.getClass().getName()+"\n"+e.getMessage());
            }
        }
	...
	public final void recalculateVisibleLines()
	{
		...
		int height = painter.getHeight();
                ...
                // get line height
                int lineHeight;
                if (painter == null || painter.getFontMetrics() == null)
	...
	public boolean scrollToCaret()
	{
        ...
        /**
         * Sets the focus to this TextArea, so this component
         * is instantly registered for keypress-events.
        ...
        public void setEditingFocus() {
            try {
                ...
                focusedComponent = this;
                setCaretPosition(0); // set caret to 0,0 coords
            }
            catch (NullPointerException e) {
                System.out.println("Nullpointer Excepion in JEditTextArea.setEditingFocus()");
            }
        }
		...
		else
		{
			...
			for(;;)
			{
				...
				if(id == Token.NULL)
					...
					fm = styles[id].getFontMetrics(defaultFont, painter.getGraphics());
